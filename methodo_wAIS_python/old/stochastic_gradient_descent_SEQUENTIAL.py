...
def SGA_L( f : DistributionFamily ,q : DistributionFamily , N : int, 𝛾 : int, η_0 : float, θ_0 : Optional[ArrayLike] = None, ɛ : float = 1e-6, iter_limit = 100, benchmark : bool = False, alpha : int = 10) -> ArrayLike:
    ...
    𝛼           - safety coefficient : if ‖ 𝛁L ‖ > 𝜶 ‖ θ_t ‖
                                        ↪ we use 𝜶 × (𝛁L / ‖ 𝛁L ‖)
    """
    ...
    while cond_n_de_suite__update_state(norm_grad_L > ɛ, state):
